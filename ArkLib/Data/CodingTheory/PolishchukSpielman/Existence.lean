...
lemma ps_exists_qx_of_cancel {F : Type} [Field F]
  ...
  · -- cardinality bound
    ...
    have hsplit : Z_x.card + Q_x.card = P_x.card := by
      exact Finset.card_filter_add_card_filter_not fun x ↦ evalX x A = 0
...
lemma ps_exists_qy_of_cancel {F : Type} [Field F]
  ...
  · -- cardinality bound
    ...
    have hsum : bad.card + Q_y.card = P_y.card := by
      exact Finset.card_filter_add_card_filter_not fun y ↦ evalY y A = 0
...
lemma ps_exists_p_nonzero {F : Type} [Field F]
  ...
  have hcard_Px' : (n_x : ℕ) - g_x ≤ Px'.card := by
    have hpart :
      ...
      simpa [Px'] using
        (Finset.card_filter_add_card_filter_not (s := P_x)
  ...
  have hcard_Py' : (n_y : ℕ) - g_y ≤ Py'.card := by
    have hpart :
      ...
      simpa [Py'] using
        (Finset.card_filter_add_card_filter_not (s := P_y)
