...
theorem card_filter_univ_subtype_eq_card_filter {α : Type} [Fintype α] [DecidableEq α]
  (s : Finset α)
  (P : α → Prop) [DecidablePred P] :
  (Finset.filter (α := s) (fun x : s => P (x : α)) Finset.univ).card =
    (s.filter P).card := by
  classical
  let t₁ : Finset s :=
    Finset.filter (α := s) (fun x : s => P (x : α)) Finset.univ
  let t₂ : Finset α := s.filter P
  have hcard : Fintype.card t₁ = Fintype.card t₂ := by
    classical
    refine Fintype.card_congr ?_
    refine
      { toFun := fun x =>
          by
            -- extract P from x.property without simp rewriting it to True
            have hxP : P (x.1 : α) := by
              -- x.2 : x.1 ∈ t₁
              -- unfold t₁ as filter and use mem_filter
              have hx' : x.1 ∈ Finset.univ ∧ P (x.1 : α) :=
                (Finset.mem_filter.mp (show x.1 ∈ t₁ from x.2))
              -- hx'.1 is x.1 ∈ univ, hx'.2 is P ...
              exact hx'.2
            refine ⟨(x.1 : α), ?_⟩
            -- show (x.1 : α) ∈ t₂
            -- membership in s.filter P is x.1 ∈ s and P x.1
            have hxmem : (x.1 : α) ∈ s := x.1.2
            exact (Finset.mem_filter.mpr ⟨hxmem, by simpa using hxP⟩)
        invFun := fun y => by
          -- y : t₂
          have hy : y.1 ∈ s ∧ P y.1 := (Finset.mem_filter.mp (show y.1 ∈ t₂ from y.2))
          let x0 : s := ⟨y.1, hy.1⟩
          refine ⟨x0, ?_⟩
          -- show x0 ∈ t₁
          -- t₁ is filter on univ
        ...
        right_inv := by
          ...
          apply Subtype.ext
          rfl }
  have : t₁.card = t₂.card := by
    simpa [Fintype.card_coe] using hcard
  simpa [t₁, t₂] using this

def curveEval {ι : Type} {F : Type} [Field F] {l : ℕ} (u : Fin (l + 2) → ι → F) (z : F) : ι → F :=
  fun x => ∑ i : Fin (l + 2), z ^ i.1 * u i x

theorem curveEval_eq_sum_smul {ι : Type} [Fintype ι] [Nonempty ι]
{F : Type} [Field F]
{l : ℕ} (u : Fin (l + 2) → ι → F) (z : F) :
  curveEval (ι := ι) (F := F) (l := l) u z =
    ∑ i : Fin (l + 2), z ^ i.1 • u i := by
  funext x
  simp [curveEval]


theorem exists_codeword_of_agree_set_ge {ι : Type} [Fintype ι] [Nonempty ι]
...
(w : ι → F) (V : Finset (ι → F)) [Nonempty V] {α : ℝ≥0} :
  ...
  intro hα
  classical
  have hmem : agree_set μ w V ∈ Finset.image (agree μ w) V := by
    unfold agree_set
    exact Finset.max'_mem _ _
  rcases Finset.mem_image.1 hmem with ⟨v, hvV, hv_eq⟩
  refine ⟨v, hvV, ?_⟩
  exact hv_eq.symm ▸ hα


theorem polynomialCurveFinite_eq_image_curveEval {ι : Type} [Fintype ι] [Nonempty ι] [DecidableEq ι]
{F : Type} [Field F] [Fintype F] [DecidableEq F]
{l : ℕ} (u : Fin (l + 2) → ι → F) :
  Curve.polynomialCurveFinite (F := F) (A := F) u =
    Finset.univ.image (fun z : F => curveEval (ι := ι) (F := F) (l := l) u z) := by
  classical
  ext w
  simp [Curve.polynomialCurveFinite, curveEval_eq_sum_smul, eq_comm]

theorem goodParams_card_ge_goodCurvePts_card {ι : Type} [Fintype ι] [Nonempty ι] [DecidableEq ι]
{F : Type} [Field F] [Fintype F] [DecidableEq F]
{l : ℕ} (u : Fin (l + 2) → ι → F)
{μ : ι → Set.Icc (0 : ℚ) 1} {deg : ℕ} {domain : ι ↪ F} {α : ℝ≥0} :
  let f : F → ι → F := fun z => curveEval (ι := ι) (F := F) (l := l) u z
  let curve : Finset (ι → F) := Curve.polynomialCurveFinite (F := F) (A := F) u
  let goodCurvePts : Finset (ι → F) := curve.filter (fun w => agree_set μ w (finCarrier domain deg) ≥ α)
  let goodParams : Finset F := Finset.univ.filter (fun z => agree_set μ (f z) (finCarrier domain deg) ≥ α)
  goodCurvePts.card ≤ goodParams.card := by
  classical
  dsimp
  -- predicate for being a good curve point
  set Good : (ι → F) → Prop := fun w => agree_set μ w (finCarrier domain deg) ≥ α with hGood
  -- rewrite the curve as an image of the evaluation map
  rw [polynomialCurveFinite_eq_image_curveEval (ι := ι) (F := F) (l := l) u]
  -- name the relevant finsets
  set goodCurvePts : Finset (ι → F) :=
      (Finset.univ.image (fun z : F => curveEval (ι := ι) (F := F) (l := l) u z)).filter Good with hgoodCurvePts
  set goodParams : Finset F :=
      Finset.univ.filter (fun z : F => Good (curveEval (ι := ι) (F := F) (l := l) u z)) with hgoodParams

  -- build a surjection goodParams → goodCurvePts
  let g : (↥goodParams) → (↥goodCurvePts) := fun z =>
    ⟨curveEval (ι := ι) (F := F) (l := l) u z.1, by
      -- extract goodness of the parameter
      have hz :
          z.1 ∈ Finset.univ.filter
            (fun z : F => Good (curveEval (ι := ι) (F := F) (l := l) u z)) := by
        simpa [hgoodParams] using z.2
      rcases Finset.mem_filter.mp hz with ⟨_, hzGood⟩
      -- membership in the image part
      have himage :
          curveEval (ι := ι) (F := F) (l := l) u z.1 ∈
            Finset.univ.image (fun z : F => curveEval (ι := ι) (F := F) (l := l) u z) := by
        refine Finset.mem_image.mpr ?_
        exact ⟨z.1, Finset.mem_univ _, rfl⟩
      have :
          curveEval (ι := ι) (F := F) (l := l) u z.1 ∈
            (Finset.univ.image (fun z : F => curveEval (ι := ι) (F := F) (l := l) u z)).filter Good :=
        Finset.mem_filter.mpr ⟨himage, hzGood⟩
      simpa [hgoodCurvePts] using this
    ⟩

  have hg : Surjective g := by
    intro w
    -- unpack membership of w in goodCurvePts
    have hw :
        w.1 ∈ (Finset.univ.image (fun z : F => curveEval (ι := ι) (F := F) (l := l) u z)).filter Good := by
      simpa [hgoodCurvePts] using w.2
    rcases Finset.mem_filter.mp hw with ⟨hwImage, hwGood⟩
    rcases Finset.mem_image.mp hwImage with ⟨z, _, hz⟩
    -- show z is a good parameter
    have hzParam : z ∈ goodParams := by
      have :
          z ∈ Finset.univ.filter
            (fun z : F => Good (curveEval (ι := ι) (F := F) (l := l) u z)) := by
        refine Finset.mem_filter.mpr ?_
        refine ⟨Finset.mem_univ z, ?_⟩
        -- use hz : curveEval u z = w
        simpa [hz] using hwGood
      simpa [hgoodParams] using this
    refine ⟨⟨z, hzParam⟩, ?_⟩
    apply Subtype.ext
    simpa [g, hz]

  have hcard : Fintype.card (↥goodCurvePts) ≤ Fintype.card (↥goodParams) :=
    Fintype.card_le_of_surjective g hg

  -- convert back to finset card (or use # notation if already present)
  simpa using hcard


theorem prob_uniform_finset_eq_card_filter_div_card {α : Type} [Fintype α] [DecidableEq α]
  (s : Finset α) [Nonempty s]
  (P : α → Prop) [DecidablePred P] :
  Pr_{let x ←$ᵖ s}[P (x : α)] =
    ((s.filter P).card : ℝ≥0) / (s.card : ℝ≥0) := by
  classical
  -- Use the lemma for the uniform distribution on the *type* `↥s`.
  simpa only [card_filter_univ_subtype_eq_card_filter (s := s) (P := P), Fintype.card_coe, Nat.cast_ofNat]
    using (prob_uniform_eq_card_filter_div_card (F := (s : Type)) (P := fun x : (s : Type) => P (x : α)))

theorem weighted_correlated_agreement_for_parameterized_curves_exists_v [DecidableEq ι] [Fintype ι] [DecidableEq F] [Fintype F]
  {l : ℕ}
  {k : ℕ} {u : Fin (l + 2) → ι → F}
  {deg : ℕ} {domain : ι ↪ F} {δ : ℝ≥0}
  {μ : ι → Set.Icc (0 : ℚ) 1}
...
theorem weighted_correlated_agreement_for_parameterized_curves [DecidableEq ι] [Fintype ι] [DecidableEq F] [Fintype F]
  ...
  classical
  intro hα hα₁ hproximity h_additionally

  obtain ⟨v, hvcode, hvμ⟩ :=
    weighted_correlated_agreement_for_parameterized_curves_exists_v
      (F := F) (ι := ι) (l := l) (k := k) (u := u) (deg := deg) (domain := domain)
      (δ := δ) (μ := μ) (M := M) (α := α) hμ hα hα₁ hproximity h_additionally

  let ι' : Finset ι := Finset.univ.filter (fun x : ι => ∀ i, u i x = v i x)
  refine ⟨ι', v, hvcode, ?_, ?_⟩
  ·
    have hset : (ι' : Set ι) = {x : ι | ∀ i, u i x = v i x} := by
      ext x
      simp [ι']
    simpa [hset] using hvμ
  ·
    intro i x hx
    have hx' : ∀ j, u j x = v j x := by
      simpa [ι'] using hx
    exact hx' i
