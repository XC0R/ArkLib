...
theorem whir_rbr_soundness
  ...
  -- where δᵣ denotes the relative Hamming distance
    ...
    {Qₛᵢ} (Oₛᵢ : OracleContext Qₛᵢ (ReaderM (OracleStatement (ι 0) F ()))) :
    ∃ n : ℕ,
    ...
    -- dtumad: I think the output oracle interface here might be wrong
      ∃ π :
        VectorIOP Unit (OracleStatement (ι 0) F ()) Unit vPSpec F Oₛᵢ Oₛᵢ.spec,
