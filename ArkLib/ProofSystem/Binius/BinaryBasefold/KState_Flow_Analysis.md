# Binary Basefold: KState & Relation Flow Analysis

## Document Purpose
This document analyzes different approaches to structuring KState and Relations 
for the RBR (Round-by-Round) knowledge soundness proofs in Binary Basefold.

**Key Finding:** Option D (simplified intermediate steps) is **REJECTED** due to 
security gaps and proof impossibilities. The original design with oracle props at 
every step has Case 4 issues at commit step. A new approach is needed.
...
## 📝 Important Note: Proposition 4.20 vs 4.22

**Proposition 4.20 (Per-step bad event bound):**
For each i ∈ {0, ϑ, ..., ℓ-ϑ}: μ(E_i) ≤ ϑ · |S^(i+ϑ)|/|L| = ϑ · 2^(ℓ+𝓡-(i+ϑ))/|L|

**Proposition 4.22 (Aggregated bad event bound):**
Is the **union bound (sum)** of all Prop 4.20 errors: Σ_i μ(E_i) ≤ 2^(ℓ+𝓡)/|L|

**Implication for RBR:**
- Each individual fold/commit step's **knowledge error** should follow **Prop 4.20**
- The **total protocol error** is the sum of per-step errors, naturally yielding **Prop 4.22**
- Prop 4.23 (proximity gap) applies at Query Phase only
...
### Flaw 1: Missing Proposition 4.20/4.22 Error Bound
...
**IMPORTANT: Prop 4.20 vs Prop 4.22 Relationship**
- **Proposition 4.20 (Per-step bound):** For each i ∈ {0, ϑ, ..., ℓ-ϑ}, μ(E_i) ≤ ϑ · |S^(i+ϑ)|/|L|
- **Proposition 4.22 (Aggregated bound):** Is the union bound (sum) of all Prop 4.20 errors: Σ_i μ(E_i) ≤ 2^{ℓ+𝓡}/|L|

For RBR, each individual fold/commit step's knowledge error should be **Prop 4.20** (the per-step bound).
The total protocol error naturally becomes **Prop 4.22** through RBR's summation of per-round errors.

**With Option D:**
- Sumcheck error: ✅ captured at fold steps
- Prop 4.20 per-step bad event error: ❌ **NOT CAPTURED** - no extraction failure when bad event occurs
...
| Fold | RBR KS Error | - | ⚠️ | Schwartz-Zippel only; **CAN'T check Prop 4.20** (no new oracle yet!) |
...
| Commit | RBR KS Error | - | ⚠️ | 0 (no challenge); **but Prop 4.20 should be here!** |
...
- If `foldingBadEventExists`: some oracle caused bad event → **bounded by Prop 4.20 per step, Prop 4.22 in total**
- If neither (some oracle not compliant, no bad event): bounded by Prop 4.23

**Critical insight:** The paper's analysis bounds bad events SEPARATELY:
- **Prop 4.20** gives per-step bound: μ(E_i) ≤ ϑ · |S^(i+ϑ)|/|L| for each i
- **Prop 4.22** is the union bound (sum) of all Prop 4.20 errors
- **Prop 4.23** applies CONDITIONAL on no bad events

For RBR: each fold step's knowledge error should be Prop 4.20; the sum yields Prop 4.22.
...
**CRITICAL: Prop 4.20 vs Prop 4.22**
- **Prop 4.20** bounds EACH individual bad event E_i: μ(E_i) ≤ ϑ · |S^(i+ϑ)|/|L|
- **Prop 4.22** is the UNION BOUND (sum) of all Prop 4.20: Σ_i μ(E_i) ≤ 2^{ℓ+𝓡}/|L|

For RBR, each fold/commit step should have knowledge error from **Prop 4.20**.
The sum over all steps naturally yields **Prop 4.22**.

**RBR approach:**
```
Pr[extraction fails] = ∑ over rounds i: Pr[¬KState(i) ∧ KState(i+1)]
                     = ∑ (Sumcheck error at i) + ∑ (Prop 4.20 at i) + (Prop 4.23)
                     = (2·ℓ)/|L|              + Prop 4.22          + ((1/2)+1/(2·2^𝓡))^γ
```
...
| Query Phase | ✅ Works | ⚠️ Missing Prop 4.20 per-step errors |
| Prop 4.20/4.22 Error | ⚠️ Unclear where captured | ❌ **NOT CAPTURED** (Prop 4.20 per-step, Prop 4.22 is sum) |
...
2. **Final sumcheck + security analysis needs oracle props IN KState**
   ...
   - Otherwise Prop 4.20/4.22 error not captured

---

## ⚠️ CRITICAL: Prop 4.20 Error Attribution Problem

**The Timeline Issue:**

1. **Fold Step (round i)**: V→P sends challenge r_i'
   - We have: old oracle f^(i) (already committed)
   - We have: the new challenge r_i'
   - We **DON'T** have: the new oracle f^(i+ϑ) yet

2. **Commit Step (round i+ϑ)**: P→V sends oracle f^(i+ϑ)
   - We have: old oracle f^(i)
   - We have: all challenges r_i', r_{i+1}', ..., r_{i+ϑ-1}'
   - We **NOW** have: the new oracle f^(i+ϑ)

**The Bad Event E_i (Prop 4.20):**

E_i involves checking: `fold(f^(i), r_i', ...) ≈ f^(i+ϑ)` or appears close to C^(i+ϑ)

This check **requires the new oracle f^(i+ϑ)**, which is only revealed at the commit step!

**The RBR Mismatch:**

| Step | Has Challenge? | Has New Oracle? | Can Check Bad Event? |
|------|---------------|-----------------|---------------------|
| Fold | ✅ Yes (r_i') | ❌ No | ❌ No |
| Commit | ❌ No (P→V only) | ✅ Yes | ✅ Yes (but no randomness!) |

In RBR, knowledge error = Pr[extraction failure | challenge at this step]

- **Fold step**: Has challenge (randomness for Prop 4.20), but CAN'T evaluate bad event (no new oracle)
- **Commit step**: CAN evaluate bad event (has all info), but has NO challenge (no randomness source)

**Conclusion: Prop 4.20 error CANNOT be correctly attributed to fold step in standard RBR!**

**Possible Resolutions:**

1. **Macro-step approach**: Treat [ϑ Fold steps + Commit step] as a single RBR round
   - The "macro step" has both challenges AND oracle revelation
   - Error is attributed to the combined step
   
2. **Separate bad event analysis**: 
   - Don't use RBR for Prop 4.20 error
   - Handle bad event probability separately (as the paper does)
   - RBR only captures sumcheck error (2ℓ/|L|) and proximity gap (Prop 4.23)
   
3. **Forward-looking KState**:
   - At fold step, KState includes "predicted" bad event based on challenge
   - The prediction is verified when oracle is revealed
