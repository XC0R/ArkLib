...
def OracleStatement : Fin (m + 1) → Type :=
  fun _ => (ω.subdomainNatReversed 0).toFinset → F
...
instance : ∀ j, OracleInterface (OracleStatement (ω := ω) m j) :=
  fun _ => inferInstance
...
def inputRelation :
    Set
      (
        (Unit × (∀ j, OracleStatement (ω := ω) m j)) ×
        Witness F s d m
      ) := sorry
...
noncomputable def batchProver :
  OracleProver []ₒ
    Unit (OracleStatement (ω := ω) m) (Witness F s d m)
    ((Fin m → F) × Fri.Spec.Statement F (0 : Fin (k + 1)))
      (OracleStatement (ω := ω) m) (Fri.Spec.Witness F s d (0 : Fin (k + 2)))
    (batchSpec F m) where
  PrvState
  | 0 => (∀j, OracleStatement (ω := ω) m j) × Witness F s d m
  | 1 => (Fin m → F) × (∀j, OracleStatement (ω := ω) m j) × Fri.Spec.Witness F s d (0 : Fin (k + 2))
...
noncomputable def batchVerifier :
  OracleVerifier []ₒ
    Unit (OracleStatement (ω := ω) m)
    ((Fin m → F) × Fri.Spec.Statement F (0 : Fin (k + 1)))
    (OracleStatement (ω := ω) m)
    (batchSpec F m) where
