...
def trueTarget (poly : CMvPolynomial n R) {i : ℕ} (fixed : Vector R i) (D : Fin m → R) : R :=
  (Finset.univ : Finset (Fin (n - i) → Fin m)).sum (fun z =>
    CMvPolynomial.eval (evalPoint (n := n) (m := m) fixed D z) poly)
...
structure RoundPolyBackend (poly : CMvPolynomial n R) (D : Fin m → R) (i : ℕ) where
...
theorem singleRoundVerifier_accept_of_backend
    (poly : CMvPolynomial n R) (D : Fin m → R) (i : ℕ)
