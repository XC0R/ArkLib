...
\begin{enumerate}
  ...
  \item any additional randomness of the ambient oracle computation (shared-oracle randomness).
\end{enumerate}
...
\]
where the probability is over the joint experiment of outside challenge sampling and any shared
oracle randomness used by $P$ and $V$.
\end{definition}
...
\begin{enumerate}
  ...
  \[
    \Pr\bigl[V(x,\tau)\in L_{\mathsf{out}}\bigr]=0.
  \]
\end{enumerate}
\end{definition}
...
every challenge position $i \in \mathcal{I}(\pSpec)$,
\[
  \Pr\Bigl[
  ...
  \ \le\ \delta(i),
\]
where $\tau$ is the transcript produced by $P$ in the joint experiment.
\end{definition}
...
\[
  A \defeq [\,\exists s \in L_{\mathsf{out}},\ V(x,\tau)=\mathsf{some}(s)\,].
\]
By initialization, $\mathsf{SF}(0,x,\langle\rangle)=\mathsf{false}$. By terminal soundness, we have
the set inclusion
\[
  A \subseteq [\,\mathsf{SF}(L,x,\tau)=\mathsf{true}\,].
\]
...
\begin{enumerate}
  ...
  \]
  \item (\textbf{Finalization}) For all $x$, full transcripts $\tau$, and output witnesses
  $w_{\mathsf{out}}\in\WitOut$, if
  \[
    \Pr\bigl[(V(x,\tau), w_{\mathsf{out}})\in rel_{\mathsf{out}}\bigr] > 0,
  \]
  then
  \[
...
statement $x$, every prover $P$, and every challenge position $i \in \mathcal{I}(\pSpec)$,
\[
  \Pr\Bigl[
...
\begin{align*}
  ...
  E(x,w_{\mathsf{out}},\tau) &\defeq w_0 \in \WitIn.
\end{align*}
...
On the other hand, if $V$ accepts with an output witness in $rel_{\mathsf{out}}$ with nonzero
probability (which is the case on the event $B$), then by the finalization property we have
$\mathsf{KSF}(L,x,\tau,w_L)=\mathsf{true}$ where $w_L=\mathsf{extOut}(x,\tau,w_{\mathsf{out}})$.
...
\[
  B \subseteq \bigcup_{i\in\mathcal{I}(\pSpec)} F_i,
\]
...
\[
  \Pr[B] \le \sum_{i\in\mathcal{I}(\pSpec)} \Pr[F_i] \le \sum_{i\in\mathcal{I}(\pSpec)} \kappa(i),
\]
as desired.
\end{proof}
